...
├── scanner\\\_core.py       ← 你的原始掃描程式（原封不動）
├── templates/
│   ├── login.html        ← 登入頁面
...
```python
\\\_DEFAULT\\\_TRIAL\\\_PASSWORDS = \\\['trial2024', 'demo888']   # ← 試用密碼（1天）
\\\_DEFAULT\\\_FULL\\\_PASSWORDS  = \\\['stock2024vip', 'bob0309']  # ← 正式密碼（180天）
```
...
|變數名|範例值|說明|
|-|-|-|
|`TRIAL\\\_PASSWORDS`|`trial2024,demo888`|試用密碼，逗號分隔，有效 1 天|
|`FULL\\\_PASSWORDS`|`stock2024vip,bob0309`|正式密碼，逗號分隔，有效 180 天|
|`SECRET\\\_KEY`|（隨機產生）|Flask session 簽名金鑰|
